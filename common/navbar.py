...
def get_navbar_items(is_homepage):
    ...
    items += [
        NavbarItem(
            title=sp.navbar_title,
            url=reverse("static-page", kwargs={"permalink": sp.permalink}),
        )
        for sp in StaticPage.objects.filter(
            navbar_title__isnull=False, is_published=True
        ).order_by("navbar_serial", "created")
    ]
