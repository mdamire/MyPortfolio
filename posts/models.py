...
class PostTag(TimeStampedModel):
    ...
    color = models.CharField(
        max_length=20, default='white', help_text="css color (hex, rgb or color name) for text color"
    )
    bg_color = models.CharField(
        max_length=20, default='grey', help_text="css color (hex, rgb or color name) for background color",
    )
